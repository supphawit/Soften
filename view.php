...
if(!isset($_GET['showModal'])){
  header("location: view.php?showModal=0");
}else if($_GET['showModal'] ==0){
  $_GET['showModal'] == 0;
}else{
  $_GET['showModal'] ==1;
}
?>
  ...
  <body style="background-color:#fff3ed">
    ...
    <!--Detail-->
    <?php
      $detail_news = $pdo->prepare("SELECT * FROM news WHERE news_id LIKE ".$_GET['news_id']);
      $detail_news->execute(); 
      $load = 0;
      while($tableDetails = $detail_news->fetch()){
          echo "<div class='card contentMargin' style='width: 800px;'>";
          echo "<div class='card-header'><br>";

          // Title
          echo "<h3>".$tableDetails['title']."</h3>";
          // Time
          echo "<div class = 'fontTime'>โพสเมื่อวันที่ : ".$tableDetails['time']."</div>";
          // Content
          echo "<br>";
          echo "&nbsp &nbsp &nbsp &nbsp &nbsp &nbsp <h7>".$tableDetails['content']."</h7>";
          // Picture
            echo "<br><br><br>";
            // echo "<img src='img/m" .$tableDetails['news_id']. ".jpg'alt='picture' height='450px' width='600px'> </img>";
            //DEMO
            $result = $pdo->query("SELECT news.news_id,pic_link FROM news LEFT JOIN pictures ON pictures.news_id = news.news_id WHERE news.news_id = '".$_GET['news_id']."'");
                while($row = $result->fetch()){
                  if($row["pic_link"]){
                  echo "<div align='center'>";
                  echo "<img src='".$row["pic_link"]."' height='450px' width='600px'><br><br>";
                  echo "</div>";
                  }
                } 
            echo "<br><br>";
      ...
      echo "<div class='row'>";
        $btn_new = $pdo->query("SELECT * FROM news ORDER BY news_id DESC LIMIT 0,1");
        $btn_old = $pdo->query("SELECT * FROM news ORDER BY news_id ASC LIMIT 0,1");
        $new_btn = $btn_new->fetch();
        $old_btn = $btn_old->fetch();
          if($new_btn["news_id"]==$_GET['news_id']){
            // Next
            echo "<div align='right' class='moreMargin'>";
            $pagePrev = $_GET['news_id'];
            $pagePrev--;
            echo "<a href='view.php?showModal=0&news_id=".$pagePrev."'><button type='button'  id='previous' class='btn btn-pink ' >Previous</button></a>";
            echo "</div>";
          }elseif($old_btn["news_id"]==$_GET['news_id']){
            // Previous
            echo "<div class='col-10'></div>";
            echo "<div align='left' class='moreMarginLeft'>";
            $pageNext = $_GET['news_id'];
            $pageNext++;
            echo "<a href='view.php?showModal=0&news_id=".$pageNext."'><button type='button'  id='next' class='btn btn-pink ' >Next</button></a>";
            echo "</div>";
