      ...
      <table class="table table-hover" style="margin-top:20px;margin-bottom:20px;">
        <?php
          if(!isset($_SESSION['username'])){
            $publicArticle = $pdo->prepare("SELECT * FROM articles WHERE category='travel' AND status = 'public' ORDER BY articles.datetime ASC");
            ...
            $memberArticle = $pdo->prepare("SELECT * FROM articles WHERE category='travel' AND (status = 'public' OR status = 'member') ORDER BY articles.datetime ASC");
